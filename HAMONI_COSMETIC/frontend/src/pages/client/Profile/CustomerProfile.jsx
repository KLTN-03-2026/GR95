...
const CustomerProfile = () => {
    const [formData, setFormData] = useState({
        MaKH: '', HoTen: '', Email: '', SoDienThoai: '', 
        GioiTinh: '', NgaySinh: '', DiaChi: '', AvatarUrl: ''
    });
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                ...
                const ngaySinh = formatDateForInput(user.ngaySinh || cachedUser.ngaySinh);
                setFormData({
                    MaKH: user.id || '',
                    HoTen: user.hoTen || '',
                    Email: user.email || '',
                ...
                loginSuccess({
                    ...
                    id: user.id || '', name: user.hoTen || '', hoTen: user.hoTen || '',
                    email: user.email || '', soDienThoai: user.soDienThoai || '',
                    gioiTinh: user.gioiTinh || '', ngaySinh: ngaySinh, diaChi: user.diaChi || '', avatarUrl
                });
                setLoading(false);
            } catch (error) {
    ...
    const handleAvatarChange = async (e) => {
        ...
        if (!allowed.includes(file.type)) {
            setAlertConfig({ show: true, type: 'danger', message: 'Chỉ chấp nhận ảnh .jpeg, .jpg, .png, .webp' });
            return;
        }
        if (file.size > 1 * 1024 * 1024) {
            setAlertConfig({ show: true, type: 'danger', message: 'Kích thước tối đa 1 MB' });
            return;
        }
        ...
        try {
            ...
            setAlertConfig({ show: true, type: 'danger', message: 'Không thể upload ảnh' });
        }
    };
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            if (!formData.HoTen.trim()) {
                setAlertConfig({ show: true, type: 'danger', message: "Vui lòng nhập họ và tên!" });
                return;
            }
            
            // Validate Email - phải có @ và .com
            if (!formData.Email.trim()) {
                setAlertConfig({ show: true, type: 'danger', message: "Vui lòng nhập email!" });
                return;
            }
            const emailRegex = /^[^\s@]+@[^\s@]+\.(com|vn|org|net|edu)$/i;
            if (!emailRegex.test(formData.Email.trim())) {
                setAlertConfig({ show: true, type: 'danger', message: "Email phải có định dạng hợp lệ (chứa @ và .com/.vn/.org/...)" });
                return;
            }
            ...
            if (formData.NgaySinh.trim()) {
                ...
                if (selectedDate >= today) {
                    setAlertConfig({ show: true, type: 'danger', message: "Ngày sinh phải trước ngày hôm nay!" });
                    return;
                }
            }
            ...
            localStorage.setItem('user_info', JSON.stringify({ 
                ...currentUserInfo, hoTen: formData.HoTen, email: formData.Email,
                soDienThoai: formData.SoDienThoai, ngaySinh: formData.NgaySinh,
                diaChi: formData.DiaChi, gioiTinh: formData.GioiTinh,
                avatarUrl: formData.AvatarUrl || currentUserInfo.avatarUrl || ''
            }));
            loginSuccess({
                ...(JSON.parse(localStorage.getItem('user')) || {}),
                ...(JSON.parse(localStorage.getItem('user_info')) || {}),
                name: formData.HoTen, hoTen: formData.HoTen, email: formData.Email,
                soDienThoai: formData.SoDienThoai, gioiTinh: formData.GioiTinh,
                ngaySinh: formData.NgaySinh, diaChi: formData.DiaChi,
                avatarUrl: formData.AvatarUrl || currentUserInfo.avatarUrl || ''
            });

            setAlertConfig({ show: true, type: 'success', message: "Cập nhật thông tin thành công!" });
        ...
        <div className="customer-profile-container py-5">
            <div className="container">
                <div className="mb-4">
                    ...
                    <p className="text-muted">Cập nhật và quản lý thông tin tài khoản của bạn</p>
                </div>
                            ...
                            <div className="card-body text-center p-5">
                                <div className="avatar-circle mx-auto mb-4">
                                    {avatarPreview ? (
                                        ...
                                        <span>{formData.HoTen?.charAt(0).toUpperCase() || 'K'}</span>
                                    )}
                                </div>
                                <div className="mb-3">
                                    <label htmlFor="avatarInput" className="btn btn-light border d-inline-block avatar-upload-btn">Chọn Ảnh</label>
                                    <input id="avatarInput" type="file" accept="image/*" style={{ display: 'none' }} onChange={handleAvatarChange} />
                                    <div className="form-text text-muted mt-2">Dung lượng tối đa 1 MB</div>
                                </div>
                                <h3 className="fw-bold text-dark mb-2">{formData.HoTen || 'Khách hàng'}</h3>
                                <p className="text-muted mb-3"><small>Mã khách hàng: <strong>KH-{formData.MaKH}</strong></small></p>
                                
                                <hr className="my-4 opacity-25" />
                                
                                <div className="text-start">
                                    <div className="mb-3 d-flex align-items-center gap-2 text-secondary">
                                        <Mail size={16} /> <small>{formData.Email || 'Chưa cập nhật'}</small>
                                    </div>
                                    <div className="d-flex align-items-center gap-2 text-secondary">
                                        <Phone size={16} /> <small>{formData.SoDienThoai || 'Chưa cập nhật'}</small>
                                    </div>
                                </div>
                            </div>
                        </div>
