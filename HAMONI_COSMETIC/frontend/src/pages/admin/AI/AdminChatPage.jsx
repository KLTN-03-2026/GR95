...
import { Search, Send, User, Bot, UserCog, CheckCircle, Clock, AlertCircle } from 'lucide-react';
...
const SOCKET_BASE = import.meta.env.VITE_SOCKET_URL || API_BASE.replace(/\/api\/?$/, '') || 'http://localhost:5000';
...
const isFirstLoad = useRef(true);
    const staffInfo = JSON.parse(localStorage.getItem('user')) || { id: 1, name: 'Admin Hamoni' };
    ...
    useEffect(() => {
        ...
        const fetchSessions = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    const withUnread = data.map(s => ({ ...s, unreadCount: s.unreadCount || 0 }));
        ...
        socket.on('customer_waiting', (data) => {
            ...
            setSessions(prev => {
                ...
                if (existing) {
                    return prev.map(s => s.maPhien === maPhien ? { ...s, status: 'pending', lastMessage: text } : s);
                }
                return [{ maPhien, roomName, customerName: 'Khách mới', status: 'pending', lastMessage: text, time: new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }) }, ...prev];
            });
        ...
        socket.on('monitor_message', (data) => {
            ...
            setSessions(prev => {
                ...
                const updated = prev.map(s => {
                    if (s.maPhien === data.maPhien) {
                        found = true;
                        const isActive = activeSessionRef.current && activeSessionRef.current.maPhien === data.maPhien;
                        return {
                            ...s,
                ...
                if (!found) {
                    return [{ maPhien: data.maPhien, roomName: data.roomName || `GUEST_${data.maPhien}`, customerName: 'Khách mới', status: data.status || 'pending', lastMessage: data.text, time: data.time, unreadCount: 1 }, ...prev];
                }
                return updated;
            });
            ...
            setActiveSession(currentActive => {
                if (currentActive && currentActive.maPhien === data.maPhien) {
                    ...
                    setSessions(prev => prev.map(s => s.maPhien === data.maPhien ? { ...s, unreadCount: 0 } : s));
                }
                return currentActive;
            });
        });
    ...
    const handleSelectSession = async (session) => {
        ...
        setSessions(prev => prev.map(s => s.maPhien === session.maPhien ? { ...s, unreadCount: 0 } : s));
        ...
        try {
            ...
            if (response.ok) {
                ...
                setMessages(visible);
                visible.forEach(m => { if (m.id) seenMessageIds.current.add(m.id); });
            }
        } catch (error) {
    ...
    const handleTakeover = () => {
        ...
        setSessions(prev => prev.map(s => s.maPhien === activeSession.maPhien ? updatedSession : s));
        ...
        setMessages(prev => [...prev, { id: msgId, senderType: 'STAFF', text: welcomeText, time: new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }) }]);
    ...
    const getStatusBadge = (status) => {
        const baseClass = "flex items-center justify-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0 whitespace-nowrap";
        switch (status) {
            case 'pending': return <span className={`${baseClass} text-rose-600 bg-rose-100`}><AlertCircle size={10} className="animate-pulse" /> Đang chờ</span>;
            case 'human': return <span className={`${baseClass} text-emerald-600 bg-emerald-100`}><UserCog size={10} /> Nhân viên</span>;
            case 'bot': return <span className={`${baseClass} text-indigo-600 bg-indigo-100`}><Bot size={10} /> AI</span>;
            case 'closed': return <span className={`${baseClass} text-slate-500 bg-slate-100`}><CheckCircle size={10} /> Đã xong</span>;
            default: return null;
        }
    };
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 flex h-full overflow-hidden">
                ...
                <div className="w-[340px] shrink-0 border-r border-slate-200 flex flex-col bg-slate-50/50">
                    <div className="p-4 border-b border-slate-200 bg-white">
                        <h2 className="text-lg font-bold m-0 mb-3 text-slate-800">Quản lý Inbox</h2>
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar">
                        {sessions.length === 0 ? (
                            <div className="text-center p-6 text-sm text-slate-400">Không có đoạn chat nào trong cơ sở dữ liệu.</div>
                        ) : (
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0 flex flex-col justify-center">
                                        <div className="flex justify-between items-center mb-1">
                                            <h3 className="font-bold text-sm text-slate-800 m-0 truncate pr-2">{session.customerName}</h3>
                ...
                <div className="flex-1 min-w-0 flex flex-col bg-white">
                    {activeSession ? (
                        <>
                            <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-white shadow-sm z-10">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h2 className="font-bold text-base m-0 text-slate-800">{activeSession.customerName}</h2>
                                        <p className="text-xs text-slate-500 m-0 flex items-center gap-1 mt-0.5">
                                            Phòng: {activeSession.roomName} • {getStatusBadge(activeSession.status)}
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    {activeSession.status === 'human' && (
                                        <button onClick={handleEndChat} className="bg-slate-100 hover:bg-slate-200 text-slate-600 px-4 py-2 rounded-lg text-sm font-medium transition-colors">
                                            Kết thúc hội thoại
                                        </button>
                                    )}
                                </div>
                            ...
                            <div className="flex-1 overflow-y-auto p-6 bg-slate-50/50 space-y-5 custom-scrollbar">
                                {activeSession.status === 'bot' && (
                                    <div className="bg-indigo-50 border border-indigo-100 p-3 rounded-lg text-xs text-indigo-600 text-center flex items-center justify-center gap-2 mb-4 mx-auto max-w-md shadow-sm">
                                        <Bot size={14} /> Bạn đang ở chế độ xem. AI đang tự động tư vấn cho khách hàng này.
                                    </div>
                                )}
                                ...
                                {messages.map(msg => (
                                    <div key={msg.id} className={`flex ${msg.senderType === 'USER' ? 'justify-start' : 'justify-end'}`}>
                                        ...
                                        <div className={`max-w-[70%] p-3 rounded-2xl text-[13px] shadow-sm ${
                                            msg.senderType === 'USER' 
                                            ...
                                            {msg.senderType !== 'USER' && (
                                                <div className={`text-[10px] font-bold mb-1 ${msg.senderType === 'BOT' ? 'text-indigo-600' : 'text-rose-100'}`}>
                                                    {msg.senderType === 'BOT' ? 'Hamoni AI' : staffInfo.name}
                                                </div>
                                            )}
                                            <div className="leading-relaxed whitespace-pre-wrap">{msg.text}</div>
                                            <div className={`text-[10px] mt-1.5 text-right ${msg.senderType === 'STAFF' ? 'text-rose-200' : 'text-slate-400'}`}>
                                                {msg.time}
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div className="p-4 bg-white border-t border-slate-200">
                                <form onSubmit={handleSendMessage} className="flex items-center gap-3">
                                    <input
                                        ...
                                        placeholder={activeSession.status === 'human' ? "Nhập tin nhắn hỗ trợ khách hàng..." : "Hãy bấm 'Tiếp nhận' để bắt đầu chat"}
                                    ...
                                    <button 
                                        type="submit" 
                                        disabled={!inputText.trim() || activeSession.status !== 'human'}
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center text-slate-400 bg-slate-50/30">
                            ...
                            <h3 className="font-bold text-xl text-slate-600 m-0 mb-2">Hộp thư hỗ trợ Hamoni</h3>
                            <p className="text-sm">Chọn một cuộc hội thoại bên trái để bắt đầu hỗ trợ khách hàng</p>
                        </div>
                    )}
                </div>
            </div>
...
const MessageCircle = ({ size, className }) => (
    <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M7.9 20A9 9 0 1 0 4 16.1L2 22Z"></path></svg>
);
