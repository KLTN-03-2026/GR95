...
// Vercel: VITE_API_BASE_URL=https://link-backend-render.onrender.com/api
...
const AdminChatPage = () => {
  ...
  // 2. KHI ADMIN CLICK VÀO 1 KHÁCH HÀNG
  const handleSelectSession = async (session) => {
    ...
    try {
      ...
      if (!response.ok) {
        console.error(
          "Lỗi API chi tiết chat:",
          response.status,
          response.statusText,
          `${API_URL}/chats/${session.maPhien}`,
        );
        return;
      }
            ...
            {sessions.length === 0 ? (
              ...
              sessions.map((session) => (
                <div
                  ...
                  className={`p-4 border-b border-slate-100 cursor-pointer transition-all flex gap-3 items-center ${
                    activeSession?.maPhien === session.maPhien
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="font-bold text-base m-0 text-slate-800">
                      {activeSession.customerName || "Khách hàng"}
                    </h2>
