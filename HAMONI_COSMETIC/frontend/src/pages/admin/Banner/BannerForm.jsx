...
const BannerForm = ({ isOpen, onClose, data, onSuccess }) => {
    ...
    if (isOpen) {
      ...
      const fetchActivePromos = async () => {
        try {
          ...
          const resp = await axios.get(`${API_BASE_URL}/promotions/active`);
  ...
  const validateForm = () => {
    ...
    if (
      formValues.NgayBatDau &&
      formValues.NgayHetHan &&
      formValues.NgayBatDau > formValues.NgayHetHan
    ) {
    ...
    if (checkDuplicateOrder(formValues.ThuTuHienThi)) {
      newErrors.ThuTuHienThi =
        "Số thứ tự này đã được sử dụng bởi banner đang hoạt động";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      onSuccess(
        editingId ? "Cập nhật banner thành công" : "Thêm banner mới thành công",
      );
      onClose();
    } catch (error) {
      const apiMessage =
      ...
      setAlert({
        show: true,
        type: "danger",
        message: "Lỗi khi lưu: " + (apiMessage || error.message),
      });
    }
  };
    ...
    >
      ...
      >
        ...
        <div className="card-body px-4 pb-4" style={{ overflowY: "auto" }}>
          {alert.show && (
            <div
              className={`alert alert-${alert.type} d-flex align-items-center p-3 mb-4`}
              role="alert"
            >
              {alert.type === "success" ? (
          ...
          <form onSubmit={handleSubmit} encType="multipart/form-data">
              ...
              <div className="d-flex flex-column gap-2">
                <div className="input-group">
                  ...
                  >
                    <Upload
                      size={18}
                      className={errors.image ? "text-danger" : ""}
                    />
                  </label>
                  <input
                    id="upload-banner"
                ...
                {preview && (
                  <div
                    className="rounded-3 border overflow-hidden bg-white d-flex justify-content-center align-items-center mt-2"
                    style={{ height: "180px" }}
                  >
                    <img
                      src={preview}
            ...
            <div className="row mb-3">
              <div className="col-md-6">
                ...
                <input
                  ...
                  type="text"
                  onChange={(e) =>
                    setFormValues({ ...formValues, URLDich: e.target.value })
                  }
                />
                ...
                {activePromotions && activePromotions.length > 0 ? (
                  <div className="mt-2">
                    <label className="form-label small text-muted">
                      Chọn chương trình khuyến mãi đang hoạt động
                    </label>
                    <select
                      ...
                      value={(() => {
                        const found = activePromotions.find(
                          (p) =>
                            `/khuyen-mai/${p.MaCTKM}` === formValues.URLDich,
                        );
                    ...
                    >
                      ...
                      {activePromotions.map((p) => (
                        <option key={p.MaCTKM} value={p.MaCTKM}>
                          {p.TenCTKM}
                          {p.NgayKetThuc
                            ? ` (Hết: ${formatDateDisplay(toDateInputValue(p.NgayKetThuc))})`
                            : ""}
                        </option>
                      ))}
                    </select>
                    <div className="form-text small text-muted mt-1">
                      Chọn khuyến mãi để tự điền URL, hoặc gõ URL tuỳ ý.
                    </div>
                  </div>
                ) : (
                  <div className="mt-2">
                    <small className="text-muted">
                      (Không có chương trình khuyến mãi đang hoạt động)
                    </small>
                  </div>
                )}
              </div>
            ...
            <div className="row mb-4">
                  ...
                  onChange={(e) => {
                    ...
                    if (checkDuplicateOrder(parsedValue)) {
                      setErrors({
                        ...errors,
                        ThuTuHienThi:
                          "Số thứ tự này đã được sử dụng bởi banner khác",
                      });
              ...
              <div className="col-md-6">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setFormValues({ ...formValues, TrangThai: e.target.value })
                  }
                >
            ...
            <div className="row mb-4">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (errors.NgayBatDau)
                      setErrors({ ...errors, NgayBatDau: null });
                  }}
                />
                {errors.NgayBatDau && (
                  <div
              ...
              <div className="col-md-6">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormValues({ ...formValues, NgayHetHan: e.target.value })
                  }
                />
              </div>
            </div>
            ...
            <div className="form-text">
              Để trống nếu banner không giới hạn thời gian.
            </div>
            ...
            <div className="d-flex gap-3">
              <button
                type="button"
                className="btn btn-light flex-grow-1 border-0 fw-semibold"
                onClick={onClose}
              >
                Hủy bỏ
              </button>
