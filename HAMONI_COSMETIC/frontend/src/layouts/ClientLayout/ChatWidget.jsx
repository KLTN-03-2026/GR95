...
import { MessageCircle, X, Send, Bot, UserCog, MoreHorizontal, CheckCircle } from 'lucide-react';
import io from 'socket.io-client'; 

// Kết nối đến Backend (Thay port nếu Backend chạy port khác)
// SỬA LẠI DÒNG NÀY
...
const ChatWidget = () => {
    ...
    useEffect(() => {
        ...
        const maND = userStr ? JSON.parse(userStr).id : null;

        // Gửi tín hiệu bắt đầu
        socket.emit('join_chat', { maND, sessionID });

        // Nhận mã phòng và lịch sử
       // Nhận mã phòng, lịch sử VÀ TRẠNG THÁI HIỆN TẠI
        socket.on('chat_ready', ({ maPhien, roomName, history, status }) => {
            ...
            // 1. Cập nhật giao diện dựa theo trạng thái DB gửi về
            // Nếu phòng đang là 'human' (nhân viên đang chat) hoặc 'pending' (đang chờ nhân viên)
        ...
        socket.on('receive_message', (incomingMsg) => {
            ...
            // Tự động chuyển giao diện nếu nhận được tin nhắn từ STAFF
            if (incomingMsg.senderType === 'STAFF') {
                setIsHandoverToHuman(true);
            }
        });

        // Tùy chọn nâng cao: Lắng nghe sự kiện kết thúc từ Admin
    ...
    const handleTouchMove = useCallback((e) => {
        if (isDragging) {
            setHasDragged(true);
            setPosition({ x: e.touches[0].clientX - dragStartPos.current.x, y: e.touches[0].clientY - dragStartPos.current.y });
        }
    }, [isDragging]);
    ...
    const handleRequestHuman = () => {
        ...
        // Có thể tùy chọn tự động gửi một thông báo nhỏ lên server để ghi nhận
        socket.emit('send_message', {
            maPhien: chatSession.current.maPhien,
            roomName: chatSession.current.roomName,
    ...
    return (
        ...
        >
            {isOpen && (
                <div ref={chatWindowRef} className="w-[360px] h-[540px] bg-white rounded-2xl shadow-2xl flex flex-col border border-slate-100 overflow-hidden mb-4 animate-fade-in-up relative">
                    
                    <div className={`p-4 text-white flex justify-between items-center shadow-sm ${isHandoverToHuman ? 'bg-gradient-to-r from-emerald-500 to-teal-600' : 'bg-gradient-to-r from-rose-500 to-rose-600'}`}>
                        ...
                        <button onClick={() => setIsOpen(false)} className="text-white hover:bg-white/20 p-1.5 rounded-lg transition-colors outline-none">
                            <X size={20} />
                        </button>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 bg-slate-50 space-y-4 scrollbar-thin scrollbar-thumb-slate-200">
                        {messages.map((msg) => (
                            <div key={msg.id} className={`flex ${msg.senderType === 'USER' ? 'justify-end' : 'justify-start'}`}>
                                {msg.senderType !== 'USER' && (
                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center mr-2 flex-shrink-0 mt-1 ${msg.senderType === 'AI' || msg.senderType === 'BOT' ? 'bg-indigo-100 text-indigo-600' : 'bg-emerald-100 text-emerald-600'}`}>
                                        {msg.senderType === 'AI' || msg.senderType === 'BOT' ? <Bot size={16} /> : <UserCog size={16} />}
                                    </div>
                                )}
                                <div className={`max-w-[75%] p-3 rounded-2xl text-sm shadow-sm ${
                                    ...
                                    {msg.senderType !== 'USER' && (
                                        <div className={`text-[10px] font-bold mb-1 ${msg.senderType === 'AI' || msg.senderType === 'BOT' ? 'text-indigo-600' : 'text-emerald-600'}`}>
                                            {msg.senderType === 'AI' || msg.senderType === 'BOT' ? 'Hamoni AI' : 'Nhân viên tư vấn'}
                                        </div>
                                    )}
                                    <div className="leading-relaxed whitespace-pre-wrap">{msg.text}</div>
                                    <div className={`text-[10px] mt-1 text-right ${msg.senderType === 'USER' ? 'text-rose-200' : 'text-slate-400'}`}>
                                        {msg.time}
                                    </div>
                                </div>
                            </div>
                        ...
                        {isAiThinking && (
                            <div className="flex justify-start">
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center mr-2 ${isHandoverToHuman ? 'bg-emerald-100 text-emerald-600' : 'bg-indigo-100 text-indigo-600'}`}>
                                    {isHandoverToHuman ? <UserCog size={16} /> : <Bot size={16} />}
                                </div>
                    ...
                    <div className="p-3 bg-white border-t border-slate-100">
                        {/* Khu vực trạng thái chuyển tiếp nhân viên */}
