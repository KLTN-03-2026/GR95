...
const getPublicProductDetail = async (req, res) => {
  ...
  try {
    ...
    const sortedVariants = [...variantRows].sort(
      (a, b) => Number(a.GiaBan || 0) - Number(b.GiaBan || 0),
    );
    const lowestPrice =
      sortedVariants.length > 0 ? Number(sortedVariants[0].GiaBan || 0) : 0;
    const lowestOldPrice =
      sortedVariants.length > 0 && sortedVariants[0].GiaGoc != null
  ...
  try {
    ...
    if (Number(review.MaND) !== userId) {
      return res
        .status(403)
        .json({
          message: "Bạn chỉ có thể phản hồi bài đánh giá của chính mình.",
        });
    }
    ...
    if (purchaseRows.length === 0) {
      return res
        .status(403)
        .json({ message: "Bạn chưa có đơn hàng hoàn tất cho sản phẩm này." });
    }
    ...
    if (shopReplyRows.length === 0) {
      return res
        .status(400)
        .json({
          message: "Chỉ phản hồi lại khi shop đã trả lời đánh giá của bạn.",
        });
    }
...
const addProductImage = async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Đã thêm ảnh!", MaHinhAnh: result.insertId });
  } catch (error) {
    res.status(500).json({ message: "Lỗi Server!" });
  }
};
...
const addProductVariant = async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Đã thêm biến thể!", MaBienThe: result.insertId });
  } catch (error) {
    res.status(500).json({ message: "Lỗi Server!" });
  }
};
...
const deleteProductVariant = async (req, res) => {
  ...
  try {
    ...
    await db.execute("DELETE FROM BienTheSanPham WHERE MaBienThe = ?", [
      variantId,
    ]);
  ...
  try {
    ...
    if (usedInOrderRows.length > 0) {
      return res.status(409).json({
        message:
          "Không thể xóa sản phẩm đã phát sinh đơn hàng. Hãy ẩn sản phẩm thay vì xóa.",
      });
    }
    ...
    if (
      ...
      return res.status(409).json({
        message:
          "Không thể xóa vì sản phẩm vẫn còn dữ liệu ràng buộc ở bảng khác.",
      });
    }
