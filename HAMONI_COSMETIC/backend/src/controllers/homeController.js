    ...
    try {
        ...
        const [products] = await db.query(`
            SELECT 
                ...
                (
                    SELECT priced.effectivePrice
                    FROM (
                        SELECT 
                            bt.MaBienThe,
                            bt.Gia as originalPrice,
                            CASE
                    ...
                    ORDER BY priced.effectivePrice ASC, priced.MaBienThe ASC
                    LIMIT 1
                ) as price,
                (
                    SELECT priced.originalPrice
                    FROM (
                        SELECT 
                            bt.MaBienThe,
                            bt.Gia as originalPrice,
